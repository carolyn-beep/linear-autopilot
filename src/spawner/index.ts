...
class Spawner {
  ...
  private async createPullRequest(
    ...
    // Prefer the tenant-scoped GitHub token; fall back to the global one.
    const githubToken =
      (tenant as { githubToken?: string }).githubToken ?? process.env.GITHUB_TOKEN;
    const ghEnv: NodeJS.ProcessEnv = { ...scrubbedEnv() };
    if (githubToken) {
      ghEnv.GH_TOKEN = githubToken;
    }
    ...
    try {
      ...
      const prBody = redactSecrets(`## ${ticket.title}
...
🤖 Generated by Linear Autopilot`);
      ...
      // No shell: attacker-controlled ticket title/body are passed as argv, not
